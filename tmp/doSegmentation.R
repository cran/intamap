...
#	ptm<-proc.time()
	...
	print("Clustering - Segmentation")
  	...
  	xy<-coordinates(object$pointData)
  
  	#x<-xy[,1]
	#y<-xy[,2]
	#z<-object$pointData$value
	#data<-cbind(x,y,z);
	dd<-cbind(xy[,1:2],object$pointData$value)
	...
	final_index<-matrix(0,length(dd[,3]),1)
...
#	if(missing(pl)){pl=FALSE}
	...
	if(pl==TRUE){
        	#plot.point(cbind(cx,cy),jj)                 #plots the edge points			#PLOTING
		dat=cbind(cx,cy)
		plot(dat)
		points(dat[jj,1],dat[jj,2],col=2)
		points(cx[fn$np_n],cy[fn$np_n],col=3)							#PLOTING
	       	}
                      ...
                      size_cl=length(cl_ind)
                          for (k in (1+tmp):length(cl_ind_n)){
                              ...
                              cl_ind_n=rm.duplicate(cl_ind_n)
       	                      if(pl==TRUE){
				points(cx[cl_ind_n],cy[cl_ind_n],col=3)     #visualize data selection		#PLOTING
                          	}
			}
...
######################END IF INTERNAL FUNCTIONS############
	...
	if(pl==TRUE){
		for(b in 1:clust$number){
			...
			points(x[id==b],y[id==b],col=b+3,pch="*")					#PLOTING
			points(ax*xxx+b.x,ay*yyy+b.y,pch="+",col=3)			#PLOTING
		}
	}
...
long2INSP<-function(data){
	library(rgdal)
	print("Project to INSPIRE")

	dat<-SpatialPoints(data.frame(x=data[,1],y=data[,2]))
	proj4string(dat)=CRS("+proj=longlat")
	dat<-spTransform(dat,CRS("+init=epsg:3035"))		#CHECK

	return(cbind(coordinates(dat)))
}
