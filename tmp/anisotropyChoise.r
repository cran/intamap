#####################################################################################
#
#Function	: AnisotropyChoise(object)
#
...
#
#Details	: Depending the parameters specified from getIntamapParms method
#		  This function returns anisotropy parameters estimation for cases:
#
#		  doSegmentation=FALSE: The anisotropy parameters estimation for 
#		  the original dataset,	after removing distant isolated points 
#		  without any segmenation process enabled.
#
#		  doSegmentation=TRUE && singleCl=FALSE: This function returns a 
#		  matrix [cl,5] that each row represents the cluster index number,
#		  the anisotropy ratio , the anisotropy direction, the number of 
#		  cluster points and the area that in inlcuded in the convex hull
#		  of the cluster.
#
#		  doSegmentation=TRUE && singleCl=TRUE : Returns a single pair of  
#		  estimated parameters (ratio, direction ) calculated from the weighted
#		  mean of slope tensors with respect to the area each cluster
#		  includes.
#
##########################################################################################

anisotropyChoise<-function(object){
	...
	for(i in 1:clNumber){
		...
		tempPar<-estimateAnisotropySc(temp[,1],temp[,2],temp[,3])
