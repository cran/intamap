...
#howmuch<-Inf
#numpred<-length(coordinates(field)[,1])
prediction<-NULL
#if(numpred<howmuch){
tempfield<-NULL
...
prediction<-predictioncopula(tempfield,data,estimates,search,calcnew,obj$params$debug.level)
#} else {
#iter<-numpred/howmuch
#for(i in 1:ceiling(iter)){
#  tempfield<-NULL
#  if (i != ceiling(iter)) imax = i*howmuch else imax = numpred
#  tempfield$x<-coordinates(field)[(1+(i-1)*howmuch):imax,1]
#  tempfield$y<-coordinates(field)[(1+(i-1)*howmuch):imax,2]
#  tempfield$F<-as.matrix(model.matrix(delete.response(terms(obj$formulaString)),obj$predictionLocations)[(1+(i-1)*howmuch):imax,])
#
#  predlist<-as.data.frame(predictioncopula(tempfield,data,estimates,search,calcnew,obj$params$debug.level))
#  if (i == 1) {
#    prediction = predlist
#  } else {
#    prediction = rbind(prediction,predlist)
#  }
#}
#}
...
ma=max(data$z)
mi=min(data$z)
if((sum(is.na(prediction$mean[index]))>0 || max(prediction$mean[index])>ma+2*(ma-med) || min(prediction$mean[index])<mi-2*(med-mi))){
...
`predictioncopula` <-
function(locations,data,estimates,search=10,calc,debug.level){
...
newdata<-distribfunction(data$z,expected,margin$params[1])
} else {
...
for(j in 1:(search+1)){
    ijmatrix[,j]=kronecker(t(0:(2^(search-j+2)-1)%%2),matrix(1,nrow=2^(j-1),ncol=1))
}
multeps=(-1)^ijmatrix
} else {
...
data$x=xy[1,]
    data$y=xy[2,]
        xy<-matrix(c(cos(anisotropy$params[1]),sin(anisotropy$params[1]),-anisotropy$params[2]*sin(anisotropy$params[1]),anisotropy$params[2]*cos(anisotropy$params[1])),ncol=2,byrow=TRUE)%*%rbind(t(locations$x),t(locations$y))
locations$x=xy[1,]
    locations$y=xy[2,]
}

h<-as.matrix(dist(cbind(data$x,data$y)))
#hnew = t(apply(cbind(data$x,data$y),MARGIN=1,FUN = function(ppt,locations)
# spDistsN1(locations,ppt),locations = cbind(locations$x,locations$y)))

#h<-as.matrix(dist(t(as.matrix(rbind(c(data$x,locations$x),c(data$y,locations$y))))))
#hnew<-as.matrix(h[1:len,(len+1):dim(h)[1]])
#h<-h[1:len,1:len]

if(calc$mean==T){
mean1<-rep(NA,length(locations$x))
} else {
mean1<-NULL
}
...
exceedanceprob<-matrix(NA,ncol=length(calc$excprob),nrow=length(locations$x))
}
...
quantiles<-matrix(NA,ncol=numquantiles,nrow=length(locations$x))
}
if(calc$variance==FALSE){
variance<-NULL
} else {
variance<-rep(NA,length(locations$x))
}

correlation<-estimates$correlation
for(i in 1:length(locations$x)){
if(!is.nan(locations$x[i])){
if(debug.level > 0) print(i)
numpoints<-search+1
            distances=sqrt((data$x-locations$x[i])^2+(data$y-locations$y[i])^2)	#hnew[,i]
...
numpoints<-search
            ...
            sigma=covar(rbind(c(0,distances),cbind(distances,h[index,index])),correlation$params,correlation$model);
expected<-locations$F[i,]%*%estimates$trend$params

#Predictive mean
if(calc$mean==T || calc$variance==T){
if(copula$method=="chisq"){
integr=integrate(meanintchi2,0,1,copula$params,sigma,multeps,newdata[index],search,margin,expected,quantilefunction,subdivisions=100, rel.tol = .Machine$double.eps^0.25, abs.tol=1e-3,stop.on.error=FALSE)
mean1[i]<-if(integr$message=="OK"){integr$value}else{NaN}
} else {
if(copula$method=="norm"){
integr=integrate(meanintgauss,0,1,sigma,newdata[index],numpoints,margin,quantilefunction,expected,subdivisions=100, rel.tol = .Machine$double.eps^0.25, abs.tol=1e-3,stop.on.error=FALSE)
mean1[i]<-if(integr$message=="OK"){integr$value}else{NaN}
}
}
}
...
#}
#}
#quantiles[i,j]<-optim(q,findquantiles,gr=NULL,sigma,newdata[index],numpoints,calc$quantiles[j],method="L-BFGS-B",lower=q1,upper=q2)$par
...
quantiles[i,j]<-uniroot(findquantiles,c(q1,q2),f.lower=f1,f.upper=f2,tol=.Machine$double.eps^0.4,sigma=sigma,newdata=newdata[index],len=numpoints,quant=calc$quantiles[j])$root
}
if(length(margin$params)==1){
quantiles[i,]<-quantilefunction(quantiles[i,],expected,margin$params[1])
} else {
quantiles[i,]<-quantilefunction(quantiles[i,],expected,margin$params[1],margin$params[2])
}
...
exceedanceprob[i,j]<-if(integr$message=="OK"){integr$value}else{NaN}
}
} #TODO: copula$method="chisq"
}
...
if(copula$method=="norm"){
integr=integrate(varintgauss,0,1,sigma,newdata[index],numpoints,margin,quantilefunction,mean1[i],expected,subdivisions=100, rel.tol = .Machine$double.eps^0.25, abs.tol=1e-5,stop.on.error=FALSE)
variance[i]<-if(integr$message=="OK"){integr$value}else{NaN}
} #TODO: copula$method="chisq"
}
}
}
list(mean=mean1,excprob=exceedanceprob,quantiles=quantiles,variance=variance)
}

`meanintgauss` <-
function(loc,sigma,newdata,len,margin,quantilefunction,expected){
loc[loc==1]=NaN
loc[loc==0]=NaN
if(length(margin$params)==1){
...
`varintgauss` <-
function(loc,sigma,newdata,len,margin,quantilefunction,m,expected){
loc[loc==1]=NaN
loc[loc==0]=NaN
if(length(margin$params)==1){
xy<-quantilefunction(loc,expected,margin$params[1])
...
`condcopuladensgauss` <-
function(loc,sigma,newdata,len){
loc=qnorm(loc)
div=dnorm(loc)
...
`condcopuladenschi2` <-
function(loc,lambda,sigma,multeps,newdata,search){
distrib<-NULL
loc<-qchisq(loc,1,lambda)
div<-dchisq(loc,1,lambda)
#div2=prod(dchisq(newdata,1,lambda));
#pro=prod(newdata);

eps=multeps[1:2^search,1:search]*kronecker(matrix(1,nrow=2^search,ncol=1),t(sqrt(newdata)));
#c=sum(dmvnorm(eps,sqrt(lambda)*matrix(1,nrow=1,ncol=search),sigma[2:dim(sigma)[1],2:dim(sigma)[1]]))/(2^search*sqrt(pro)*div2);
...
for(i in 1:length(loc)){
    ...
    #distrib[i]=sum(dmvnorm(eps,sqrt(lambda)*matrix(1,nrow=1,ncol=search+1),sigma))/(2^(search+1)*sqrt(pro*loc[i])*div[i]*div2);
...
`meanintchi2` <-
function(loc,lambda,sigma,multeps,newdata,len,margin,expected,quantilefunction){
loc[loc==1]=NaN
loc[loc==0]=NaN
if(length(margin$params)==1){
xy<-quantilefunction(loc,expected,margin$params[1])
...
`findquantiles` <-
function(loc,sigma,newdata,len,quant){
    if(loc<1 && loc>0){
...
`exceedance` <-
function(loc,sigma,newdata,len,margin,distribfunction,densityfunction,expected){
loc[loc==1]=NaN
loc[loc==0]=NaN
if(length(margin$params)==1){
