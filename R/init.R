...
createIntamapObject = function(observations, obsChar, formulaString, predictionLocations=100,
  ...
  if ("methodParameters" %in% names(params)) {
    ...
    if (length(grep("system",methodParameters)) >0) stop("Illegal attempt to call system functions through methodParameters")
    ...
    if (length(grep("download",methodParameters)) >0) stop("Illegal attempt to download through methodParameters")
    ...
    if (length(grep("options",methodParameters)) >0) stop("methodParameters contain illegal string options")
    if (length(grep("file",methodParameters)) >0) stop("methodParameters contain illegal string file")
    if (length(grep(" q",methodParameters)) >0) stop("methodParameters contain illegal string q")
    if (length(grep("shell",methodParameters)) >0) stop("methodParameters contain illegal string shell")
